...
const char* page = "/siga/add_acesso.php";
...
void loop() {
  if (checkCard(mfrc522.uid.uidByte, mfrc522.uid.size, uid_card)) {
#ifdef cardDebug
    Serial.println("Card UID: " + uid_card);
#endif
    ...
    //Request
    POSTData = "uid_card=" + uid_card + "&id_turma=" + id_turma; // + "&id_porta=" + lockId;
...
void readTag(byte *buffer, byte bufferSize, String& uid_card) {
  uid_card = "";//Overwrite previous readed tag (if any)
  for (byte i = 0; i < bufferSize; i++) {
    uid_card.concat(String(buffer[i] < 0x10 ? "0" : ""));
    uid_card.concat(String(buffer[i], HEX));
  }
  uid_card.toUpperCase();
}

//Complete function to detect and read an RFID card UID
bool checkCard(byte *buffer, byte bufferSize, String& uid_card) {
  if (mfrc522.PICC_IsNewCardPresent()) {
    if (mfrc522.PICC_ReadCardSerial()) {
      readTag(mfrc522.uid.uidByte, mfrc522.uid.size, uid_card);
      return true;
    }
  }
  return false;
